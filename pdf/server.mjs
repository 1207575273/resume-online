/**
 * PDF 导出服务：Playwright 无头浏览器加载真实页面 → emulateMedia("screen")
 * → A4 PDF（printBackground 保住暗色/极光/视觉图）。
 * 极简 http 服务，零框架；请求串行（单浏览器复用，个人站足够）。
 */
import http from "node:http";
import { chromium } from "playwright-core";

const PORT = Number(process.env.PORT ?? 3002);
const PAGE_URL = process.env.PDF_PAGE_URL ?? "http://web:3000/";

// 系统 chromium（alpine apk 安装），非 Playwright 捆绑版
const browser = await chromium.launch({
  executablePath: process.env.CHROMIUM_PATH ?? "/usr/bin/chromium-browser",
  args: ["--no-sandbox", "--disable-dev-shm-usage"],
});
console.log(`[pdf] chromium ready → ${PAGE_URL}`);
...
async function renderPdf(theme) {
  ...
  try {
    ...
    // 入场动画直达终态 + 数字 count-up 直达终值（reducedMotion 已保证）
    await page.evaluate(() => {
      document.querySelectorAll("[data-reveal]").forEach((el) => el.classList.add("revealed"));
    });
    await page.addStyleTag({ content: PDF_FIX_CSS });
    await sleep(900); // 等字体与进度条动画收尾
...
const server = http.createServer((req, res) => {
  ...
  if (req.method !== "GET" || url.pathname !== "/resume.pdf") {
    res.writeHead(404).end("not found");
    return;
  }
  const theme = url.searchParams.get("theme") === "light" ? "light" : "dark";
  queue = queue
    .then(() => renderPdf(theme))
    .then((buffer) => {
      res.writeHead(200, {
      ...
      res.end(buffer);
      console.log(`[pdf] served ${buffer.length} bytes (${theme})`);
    })
    .catch((error) => {
      console.error("[pdf] render failed:", error);
      res.writeHead(502, { "content-type": "text/plain; charset=utf-8" });
      res.end("PDF 生成失败，请稍后重试");
    });
});
